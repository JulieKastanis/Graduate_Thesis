...
```{r modelselection__GSI_with_just_singular}
modelchroma <- lm(GSI_Value~Chroma_Gill, data = Thesis_Data)
modelsat <- lm(GSI_Value~Sat_Gill, data = Thesis_Data)
modeltint <- lm(GSI_Value~Tint_Gill, data = Thesis_Data)
...
```{r ggplot_GSI_Value}
p1 <- ggplot(Thesis_Data, aes(Chroma_Gill, GSI_Value, colour = Sat_Gill)) + 
  geom_point(size=3) +
  geom_smooth(method="lm")
...
#***Whiteness and Body Condition 

#****chroma and Sat GSI


```{r ggplot_Whiteness_BCF}
ggplot(data = Thesis_Data1, aes(x=Whiteness_Gill, y=Body_Condition_Factor)) +
geom_point()+
  xlab("Whiteness") +
    ylab("Body Condition Factor") +
theme_bw()+
...
```{r FitBCF_Whiteness}
FitBCF_Whiteness <- lm(data= Thesis_Data1, Body_Condition_Factor~Whiteness_Gill)
```

```{r  autoplot_FitBCF_Whiteness}
autoplot(FitBCF_Whiteness)
```


```{r FitBCF_Whiteness_anova}
anova(FitBCF_Whiteness)
summary(FitBCF_Whiteness)
```


```{r boxcox1}
boxcox(FitBCF_Whiteness)
```
...
```{r ggplot_chroma_GSI}
ggplot(data = Thesis_Data1, aes(x=Chroma_Gill, y=GSI_Value)) +
geom_point()+
  xlab("Chroma") +
    ylab("Gonadosomatic Index (GSI)") +
theme_bw()+
geom_smooth(method=lm)
```

```{r Fitchroma_GSI}
Fitchroma_GSI <- lm(data= Thesis_Data1, GSI_Value~Chroma_Gill)
```

```{r  autoplot_Fitchroma_GSI}
autoplot(Fitchroma_GSI)
```


```{r Fitchroma_GSI_anova}
anova(Fitchroma_GSI)
summary(Fitchroma_GSI)
```


```{r boxcox2}
boxcox(Fitchroma_GSI)
```


```{r ggplot_Sat_GSI}
ggplot(data = Thesis_Data1, aes(x=Sat_Gill, y=GSI_Value)) +
geom_point()+
  xlab("Saturation") +
    ylab("Gonadosomatic Index (GSI)") +
theme_bw()+
...
```{r Fitsat_GSI}
Fitsat_GSI <- lm(data= Thesis_Data1, GSI_Value~Sat_Gill)
```

```{r  autoplot_Fitsat_GSI}
autoplot(Fitsat_GSI)
```
...
```{r fit.sex_whiteness}
Sex_whiteness <- glm(Sex~Whiteness_Gill, data=sex, binomial(link="logit"))
...
```{r sex_whiteness_summary1}
summary(sex_whiteness)
```

```{r autoplot_sex_whiteness1}
autoplot(sex_whiteness)

```


```{r binnedplot_sex_whiteness1}
x <- predict(sex_whiteness)
y <- resid(sex_whiteness)
binnedplot(x, y)
```

```{r coeff_sex_whiteness1}
coef(sex_whiteness)
```

```{r coefint_sex_whiteness1}
confint(sex_whiteness)
```
...
```{r sex_chroma}
sex_chroma <- glm(Sex~Chroma_Gill, data=sex, binomial(link="logit"))

anova(sex_chroma)
```

```{r sex_chroma_summary}
summary(sex_chroma)
```

```{r autoplot_sex_chroma}
autoplot(sex_chroma)

```


```{r binnedplot_sex_chroma}
x <- predict(sex_chroma)
y <- resid(sex_chroma)
binnedplot(x, y)
```
...
```{r sex_area}
sex_area <- glm(Sex~Red_Coloration_Area, data=sex, binomial(link="logit"))
...
```{r sex_sat}
sex_sat <- glm(Sex~Sat_Gill, data=sex, binomial(link="logit"))
...
```{r sex_tint}
sex_tint <- glm(Sex~Tint_Gill, data=sex, binomial(link="logit"))
...
```{r sex_whiteness_summary}
summary(sex_whiteness)
```

```{r autoplot_sex_whiteness}
autoplot(sex_whiteness)

```


```{r binnedplot_sex_whiteness}
x <- predict(sex_whiteness)
y <- resid(sex_whiteness)
binnedplot(x, y)
```

```{r coeff_sex_whiteness}
coef(sex_whiteness)
```

```{r coefint_sex_whiteness}
confint(sex_whiteness)
```

```{r Binary_Sex_whiteness}
ggplot(sex, aes(Whiteness_Gill, Sex)) +
  geom_point() +
  geom_smooth(method="glm", method.args=list(family="binomial"(link="logit"))) +
  xlab ("Whiteness") +
...
```{r autoplot_season_area}
autoplot(season_whiteness)

```


```{r binnedplot_season_area}
x <- predict(season_area)
y <- resid(season_area)
binnedplot(x, y)
```
...
```{r coefint_season_area}
confint(season_area)
```

```{r Binary_Season_whiteness}
...
```{r autoplot_site_whiteness}
autoplot(site_whiteness)

```


```{r binnedplot_site_whiteness}
x <- predict(site_whiteness)
y <- resid(site_whiteness)
binnedplot(x, y)
```

```{r coeff_site_whiteness}
coef(site_whiteness)
```

```{r coefint_site_whiteness}
confint(site_whiteness)
```
...
```{r autoplot_site_chroma}
autoplot(site_chroma)

```


```{r binnedplot_site_chroma}
x <- predict(site_chroma)
y <- resid(site_chroma)
binnedplot(x, y)
```
...
```{r autoplot_site_tint}
autoplot(site_tint)

```


```{r binnedplot_site_tint}
x <- predict(site_tint)
y <- resid(site_tint)
binnedplot(x, y)
```
